...
@section('content')
            ...
            @forelse($gateways as $gateway)
                <label class="payment-method-card cursor-pointer">
                    <input type="radio" name="gateway" value="{{ $gateway->id }}" class="hidden gateway-radio" 
                        data-min="{{ $gateway->min_limit }}" 
                        data-max="{{ $gateway->max_limit }}"
                        data-currency="{{ $gateway->currency }}">
                    <div class="border-2 border-gray-200 rounded-xl p-4 text-center transition hover:border-indigo-400 hover:bg-indigo-50">
                        @if($gateway->logo_path)
                            <img src="{{ asset($gateway->logo_path) }}" alt="{{ $gateway->name }}" class="w-12 h-12 mx-auto mb-2 object-contain">
                        @else
                            <div class="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center">
    ...
    <div class="bg-white rounded-xl shadow-sm p-5 mb-5">
        ...
        <div class="flex items-center border-2 border-gray-200 rounded-xl overflow-hidden focus-within:border-indigo-500 transition">
