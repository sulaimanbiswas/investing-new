    ...
    <div class="space-y-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            @if($deposit->status === 'completed')
                                <span class="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded">success</span>
                            ...
                            @else
                                <span class="text-xs font-semibold text-gray-600 bg-gray-50 px-2 py-1 rounded">{{ $deposit->status }}</span>
                            @endif
                        </div>
                ...
                <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                    <div class="flex items-center gap-2">
                        @if($deposit->gateway && $deposit->gateway->logo_path)
                            <img src="{{ asset($deposit->gateway->logo_path) }}" alt="{{ $deposit->gateway->name }}" class="w-6 h-6 rounded-full object-cover">
                        @else
                ...
                @if($deposit->screenshot_path)
                    <div class="mt-3 pt-3 border-t border-gray-100">
                        ...
                        <a href="{{ asset($deposit->screenshot_path) }}" target="_blank" class="inline-block">
                            <img src="{{ asset($deposit->screenshot_path) }}" alt="Screenshot" class="w-32 h-32 object-cover rounded-lg border-2 border-gray-200 hover:border-indigo-500 transition">
                        </a>
                    </div>
                @endif
        ...
        @empty
            <div class="bg-white rounded-xl shadow-sm p-8 text-center">
                ...
                <a href="{{ route('deposit') }}" class="inline-block bg-indigo-600 text-white px-6 py-3 rounded-xl hover:bg-indigo-700 transition">
