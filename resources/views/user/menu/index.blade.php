...
@section('content')
    ...
    <div class="mb-5 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800">{{ __('ui.platform_menu') }}</h1>
                <p class="text-gray-500 text-sm mt-0.5">{{ __('ui.choose_level_start_earning') }}</p>
            </div>
        </div>
    </div>
        ...
        @if($hasActiveOrder)
            <div class="bg-gradient-to-br from-green-400 via-green-500 to-emerald-600 rounded-xl shadow-xl p-2">
                <div class="flex items-center gap-4 text-center">
                    <div class="flex-1">
                        <p class="text-white text-sm font-medium opacity-90 mb-1">{{ __('ui.current_boost') }}</p>
                        <p class="text-white text-xl font-bold">{{ __('ui.order_available') }}</p>
                    </div>
                </div>
            </div>
        @else
            <div class="bg-gradient-to-br from-slate-500 via-slate-600 to-gray-700 rounded-xl shadow-lg p-4">
                <div class="flex items-center justify-between gap-4 flex-wrap">
                    <div class="text-center sm:text-left flex-1 min-w-[180px]">
                        ...
                        <p class="text-white/80 text-xs mt-1">{{ __('ui.balance') }}:
                            {{ number_format((float) $user->balance, 2) }} USDT</p>
                    </div>
                    ...
                    @if($hasPendingOrderRequest)
                        <button type="button"
                            ...
                            disabled>
                            <i class="fas fa-clock mr-1"></i> {{ __('ui.requested') }}
                        </button>
                    @elseif((float) $user->balance < 20)
                        <a href="{{ route('deposit') }}"
                            ...
                            <i class="fas fa-wallet mr-2"></i> {{ __('ui.deposit') }}
                        </a>
                    @else
                        <form action="{{ route('menu.request-order') }}" method="POST" class="inline-block">
                            ...
                            <button type="submit"
                                ...
                                <i class="fas fa-paper-plane mr-2"></i> {{ __('ui.request_order') }}
                            </button>
                        </form>
                    @endif
                </div>
    ...
    <div class="grid grid-cols-4 gap-3 mb-6">
        <a href="{{ route('menu.index', ['vip_level' => 'all']) }}"
            ...
            <div class="text-xs">{{ __('ui.all') }}</div>
        </a>
        <a href="{{ route('menu.index', ['vip_level' => 'vip1']) }}"
            ...
            <div class="text-xs">{{ __('ui.level_1') }}</div>
        </a>
        <a href="{{ route('menu.index', ['vip_level' => 'vip2']) }}"
            ...
            <div class="text-xs">{{ __('ui.level_2') }}</div>
        </a>
        <a href="{{ route('menu.index', ['vip_level' => 'vip3']) }}"
            ...
            <div class="text-xs">{{ __('ui.level_3') }}</div>
        </a>
    </div>
    ...
    <div class="space-y-4">
        @forelse($platforms as $platform)
            ...
            <a href="{{ route('menu.platform.show', $platform) }}"
                class="block bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200">
                <div class="p-4">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-shrink-0">
                            ...
                            @else
                                <div
                                    class="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center shadow-sm">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2 mb-2">
                                ...
                                <!-- VIP Badge -->
                                <div
                                    class="{{ $vipColor }} text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm whitespace-nowrap">
                            ...
                            <div class="space-y-1.5">
                                <div class="flex items-center gap-2 text-sm">
                                    ...
                                    <span class="text-gray-600">{{ __('ui.balance') }}:</span>
                                    <span class="font-semibold text-gray-900">
                                        {{ number_format($platform->start_price, 0) }} -
                                        {{ number_format($platform->end_price, 0) }} USDT
                                    </span>
                                </div>
                                <div class="flex items-center gap-2 text-sm">
                                    <i class="fas fa-percentage text-green-600 w-4"></i>
                                    <span class="text-gray-600">{{ __('ui.commission') }}:</span>
        ...
        @empty
            <div class="text-center py-16">
                <div
                    class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full mb-4 shadow-inner">
                    <i class="fas fa-inbox text-gray-400 text-3xl"></i>
                </div>
                <p class="text-gray-600 text-lg font-semibold mb-1">{{ __('ui.no_platforms_available') }}</p>
                <p class="text-gray-400 text-sm">{{ __('ui.try_different_level') }}</p>
            </div>
        @endforelse
    </div>
@endsection