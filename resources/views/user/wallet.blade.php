...
@section('title', __('ui.wallet_management') . ' - ' . config('app.name'))

@section('content')
    <!-- Header with Back Button -->
    <div class="mb-6 flex items-center gap-3">
        @include('components.back-button')
        <h1 class="text-2xl font-bold text-gray-800">{{ __('ui.wallet_management') }}</h1>
    </div>
    ...
    <div class="bg-gradient-to-br from-rose-600 to-purple-700 rounded-xl shadow-lg p-5 text-white mb-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold">{{ __('ui.wallet_management') }}</h2>
                <div class="text-sm text-white/80">
                    {{ __('ui.wallet_locked_notice') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-5">
        <form id="walletForm" action="{{ route('wallet.update') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label for="wallet_name" class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.wallet_name') }}
                ...
                <input type="text" id="wallet_name" name="wallet_name"
                    ...
                    value="{{ old('wallet_name', $user->wallet_name) }}" @if($user->wallet_name) disabled @else required
                    @endif placeholder="@if($user->wallet_name) Name already set @else Enter your withdrawal name @endif">
                @error('wallet_name')
            ...
            <div>
                <label for="wallet_gateway"
                    class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.wallet_currency_protocol') }}
                    <span class="text-red-500">*</span></label>
                <select id="wallet_gateway" name="wallet_gateway"
                    ...
                    @if($user->wallet_name && $user->wallet_gateway && $user->withdrawal_address) disabled @else required
                    @endif>
                    <option value="" disabled selected>{{ __('ui.select_protocol') }}</option>
            ...
            <div>
                <label for="withdrawal_address"
                    class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.wallet_address') }} <span
            ...
            @if(!$user->wallet_name && !$user->wallet_gateway && !$user->withdrawal_address)
                <div>
                    <label for="wallet_password"
                        class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.wallet_password_verification') }}
                    ...
                    <input type="password" id="wallet_password" name="wallet_password" required
                        ...
                        placeholder="{{ __('ui.enter_wallet_password_verify') }}">
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <div class="text-sm text-yellow-800">
                            ...
                            <ul class="list-disc list-inside space-y-1 text-xs">
                                <li>{{ __('ui.once_set_wallet_unchangeable') }}</li>
                                <li>{{ __('ui.ensure_wallet_matches_protocol') }}</li>
                                <li>{{ __('ui.double_check_before_submit') }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                class="px-4 py-2 rounded-lg bg-gray-400 text-white cursor-not-allowed" @else
                    ...
                    @if($user->wallet_name && $user->wallet_gateway && $user->withdrawal_address)
                        {{ __('ui.setup_complete') }}
                    @else
                        {{ __('ui.save_wallet_details') }}
                    @endif
                </button>
            </div>
        </form>
    ...
    <div id="successModal" class="fixed inset-0 bg-black bg-opacity-60 z-[60] hidden items-center justify-center p-4">
        <div class="bg-white rounded-3xl shadow-2xl max-w-sm w-full transform transition-all">
            <div class="p-6">
                ...
                <!-- Title -->
                <h3 class="text-2xl font-bold text-gray-900 text-center mb-2" id="successTitle">{{ __('ui.success') }}</h3>
                <!-- Message -->
    ...
    <div id="errorModal" class="fixed inset-0 bg-black bg-opacity-60 z-[60] hidden items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-sm w-full transform transition-all">
            <div class="p-6">
                ...
                <!-- Title -->
                <h3 class="text-2xl font-bold text-gray-900 text-center mb-2" id="errorTitle">{{ __('ui.error') }}</h3>
                <!-- Message -->
    ...
    <script>
        document.getElementById('walletForm').addEventListener('submit', function (e) {
            ...
            submitBtn.textContent = '{{ __('ui.saving') }}';
            ...
            fetch('{{ route("wallet.update") }}', {
                ...
                .then(data => {
                    if (data.success) {
                        showSuccessModal(data.message || '{{ __('ui.save_wallet_details') }}');
                    } else {
                        showErrorModal(data.message || '{{ __('ui.error') }}');
                    }
                })
                .catch(error => {
                    showErrorModal('{{ __('ui.error_try_again') }}');
                })
                .finally(() => {
                    submitBtn.disabled = false;
                    submitBtn.textContent = '{{ __('ui.save_wallet_details') }}';
                });
        });
