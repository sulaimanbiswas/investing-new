<form method="post" action="{{ route('password.update') }}" class="space-y-5">
    ...
    <div>
        <label for="update_password_password_confirmation" class="block text-sm font-medium text-gray-700 mb-1">Confirm
            New Password</label>
    ...
    <div class="flex items-center gap-4 pt-2">
        ...
        @if (session('status') === 'password-updated')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                class="text-sm text-green-600 font-medium">✓ Password updated successfully!</p>
        @endif
    </div>
</form>